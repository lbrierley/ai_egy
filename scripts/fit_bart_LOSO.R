...
raw_sites <- raw_sites %>% bind_cols(clustsite = clust_sites)
...
foreach(exsite = 1:max(raw_sites$clustsite),
        .packages = c(
          "caret",
          "dbarts",
          "dplyr",
  ...
  train_LOSO <- df %>% filter(clustsite != exsite)
  test_LOSO <- df %>% filter(clustsite == exsite)
  
  set.seed(1400)
  #fold_ids <- caret::createFolds(train_LOSO$Result, k = 5)
  fold_ids <- caret::groupKFold(train_LOSO$clustsite, k = 5)
  
  folds <- lapply(1:length(fold_ids),
                  FUN = function(i){
                    train_LOSO[-fold_ids[[i]],] })
  ...
  basic_model <- bart( 
    x.train = train_LOSO %>% dplyr::select(all_of(preds)),
    y.train = train_LOSO %>% pull(Result),
    x.test = test_LOSO %>% dplyr::select(all_of(preds)),
    k = K_OPT,
    ntree = 4000,
    power = power_opt,
    base = base_opt,
  ...
  # Save model
  saveRDS(basic_model, file = paste0("models/LOSO/base_opt_model_", exsite, ".rds"))
  
  # Perform variable selection
  sdm <- bart.step(
    x.data = train_LOSO %>% dplyr::select(all_of(preds)),
    y.data = train_LOSO %>% pull(Result),
    k = K_OPT,
    power = power_opt,
    base = base_opt)
  ...
  # Save model
  saveRDS(sdm, file = paste0("models/LOSO/reduced_opt_model_", exsite, ".rds"))
  ...
  test_LOSO %>%
    dplyr::select(x, y, Date, clustsite, Result) %>%
    bind_cols(prob = preds_p) %>%
    write.csv(file = paste0("models/LOSO/preds_opt_model_", exsite, ".csv"))
}
...
test_LOSO_all <- list.files(path="models\\LOSO", pattern = "\\.csv", full.names=TRUE) %>% 
  map_dfr(read.csv) %>%
  dplyr::select(-X)

test_LOSO_all %>%
  ggplot(aes(x = prob, group = as.factor(Result), fill = as.factor(Result))) +
  geom_density(alpha = 0.2)

ROC = roc(response = test_LOSO_all$Result, 
          predictor = test_LOSO_all$prob,
          direction = "<")
...
test_LOSO_all <- test_LOSO_all %>% mutate(Pred = if_else(prob > cutoff, 1, 0) %>% as.factor,
                                          Result = as.factor(Result))

matrix_test <- confusionMatrix(data = test_LOSO_all$Pred, 
                               reference = test_LOSO_all$Result,
                               positive = "1")
matrix_test

test_plot <- test_LOSO_all %>%
  mutate(plotkey = paste(Result, Pred, sep = "_"))

# Performance seems to vary site-by-site!
test_plot %>% with(., table(Result, Pred, clustsite))

# metric_cis <- get_sens_and_spec_ci(sdm, 
#                                      xtest = test_LOSO %>% dplyr::select(all_of(preds)), 
#                                      ytest = test_LOSO %>% pull(Result),
#                                      cutoff)
#
